...
for figType in dataDict.keys():
    ...
    for subFigIdx in range(len(subFigK)):
        subFigCnt += 1
        plt.subplot(2, len(subFigK)//2, subFigCnt)
        ...
        plt.title(fill(('(%s) %s' % (letterArr[subFigCnt-1], subTitleDict[curFigTypeK][curSubFigK])), 60), fontsize=24, y=-0.37)  # 图标题

    # 统一的图例，参数见：https://matplotlib.org/3.1.1/api/_as_gen/matplotlib.pyplot.figlegend.html#matplotlib.pyplot.figlegend
    plt.figlegend(legendArr, legendEntryArr, ncol=len(legendEntryArr), loc="upper center", fontsize=20, columnspacing=1, handletextpad=0.3, bbox_to_anchor=(0.5, 1.01))
    plt.subplots_adjust(left=0.085, right=0.99, top=0.92, bottom=0.125, hspace=0.42) # 图的上下左右边界
