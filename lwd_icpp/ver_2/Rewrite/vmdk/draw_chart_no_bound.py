...
for line in f_result.readlines():
    ...
    if a[0]=="rewritesize":
        for b in range(1,len(a)-1):#第一个是字符，最后一个是','
            x.append(1.0*int(a[b])/(1024*1024*1024))
...
max_read_container=100000000
min_read_container=-1
...
plt.plot(smr_speed_factor,smr_rewrite_chunk,label=u'SMR',marker='*',linewidth=7)
...
plt.ylabel("Size of Rewritten Chunks (GB)",fontsize=40)
plt.savefig('../image/vmdk_rewrite_cost.png')
