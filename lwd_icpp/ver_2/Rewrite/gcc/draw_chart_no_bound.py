...
for i in range(len(dataArr)):
    ...
    plt.plot(dataArr[i][0],dataArr[i][1],label=curMethod,color=colorDict[curMethod], \
        marker=markerDict[curMethod],markersize=16,markevery=getMarkerArr(len(dataArr[i][0]), 17), \
...
plt.yticks(np.arange(0, 26, 5), fontsize=22)
...
plt.savefig('../image/gcc_rewrite_cost.pdf')
plt.show()