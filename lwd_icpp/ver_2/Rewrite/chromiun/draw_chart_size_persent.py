...
def is_near(y1,y2):
    if y2<0:
        return False
    d=total_size*(y1-y2)/(1024*1024*y1*y2)
    if d<=delt and d>=0:
        return True
    return False
...
for i in range(len(new_read_containter)-1,-1,-1):
    ...
    for j in range(0,len(capping_read_container)):
        if is_near(new_read_containter[i],capping_read_container[j]):
            near_off=j
    ...
    for j in range(0,len(smr_read_container)):
        if is_near(new_read_containter[i], smr_read_container[j]):
            near_off=j
    ...
    for j in range(0,len(fcapping_read_container)):
        if is_near(new_read_containter[i], fcapping_read_container[j]):
            near_off=j
...
plt.savefig('../image/chromiun_improve.png')
