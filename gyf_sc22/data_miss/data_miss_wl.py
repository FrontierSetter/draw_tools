...
for i in range(len(fileArr)):
    curP = plt.plot(xArr[i], dataArr[i], label=labelArr[i], lw=3, \
        color=colorArr[i], marker=markerArr[i], markevery=getMarkerArr(len(xArr[i]),15), markersize=14)
    
# 设置坐标轴文字
plt.ylabel('Wear leveling counts', fontsize=26)
...
# 设置图片边距
plt.subplots_adjust(left=0.13, right=0.99, top=0.975, bottom=0.13)

# 保存图片
plt.savefig('data_miss_wl.pdf')
