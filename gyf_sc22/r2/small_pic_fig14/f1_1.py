...
lineName = [x for x in defaultSheet.row_values(baseRow) if x != ''][1:]
print(lineName)
for i in range(len(lineName)):
    valueArr.append([])

for i in range(8):
    curArr = defaultSheet.row_values(baseRow+2+i)
    print(curArr)
    ...
    for i in range(len(lineName)):
        # print(curArr[i])
        valueArr[i].append(float(curArr[i*3+targetCol]))
...
# 保存图片
plt.savefig('.\\image\\f1_score_3.pdf')
