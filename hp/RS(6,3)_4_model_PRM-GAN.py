...
hatchDict = {'Full Recovery-PPR': '//', 'Full Recovery-RP': '\\\\', 'PRM': 'xx', 'Baseline': '--', 'GAN': '//'}
...
for subFigId in range(len(dataDict)):
    ...
    for barId in dataDict[curModelName].keys():
        ...
        offset = 0.0 - bar_width * (totalBarNum / 2.0) - gap_width * ((totalBarNum - 1.0) / 2) + (
                    barCnt + 0.6) * bar_width + barCnt * gap_width
...
plt.subplots_adjust(left=0.05, right=0.99, top=0.90, bottom=0.15)  # 图的上下左右边界

plt.savefig('RS(6,3)_4_model_GAN.pdf')
# plt.show()