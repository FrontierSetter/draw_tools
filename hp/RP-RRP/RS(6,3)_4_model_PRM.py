...
###  PPR
# colorDict = {'PRM-PPR':'darkviolet','PPR':'dodgerblue'}  #设置配色 2021-09-21
# hatchDict = {'PRM-PPR':'//','PPR':'\\\\'}

#### RP
colorDict = {'PRM-RP':'darkviolet','RP':'dodgerblue'}  #设置配色 2021-09-21
hatchDict = {'PRM-RP':'//','RP':'\\\\'}
...
## PPR
# readbook = xlrd.open_workbook('RS(6,3)数据-PRM-PPR.xlsx')

## RP
readbook = xlrd.open_workbook('RS(6,3)数据-PRM-RP.xlsx')
...
for i in range(2, len(sheets)):
    ...
    curModelName = curSheet.row_values(0)[0]
    ...
    paraDict[curModelName] = {
        ...
        'yMin': curSheet.row_values(1)[0],
        'yMax': curSheet.row_values(2)[0],
    }
...
for subFigId in range(len(dataDict)):
    ...
    for barId in dataDict[curModelName].keys():
        ...
        offset = 0.0-bar_width*(totalBarNum/1.0)-gap_width*((totalBarNum-1.0)/1)+(barCnt+0.5)*bar_width+barCnt*gap_width
...
# plt.savefig('RS(6,3)-PRM-PPR.xlsx.pdf')
plt.savefig('RS(6,3)-PRM-RP.xlsx.pdf')
# plt.show()