...
for barId in dataArr[1].keys():
    ...
    plt.bar(ind+offset, dataArr[1][barId], bar_width, label=barId, color='white', linewidth=2, edgecolor=colorDict[barId], hatch='///////' if barId == 'PRM-RP'  else '\\\\\\\\\\\\\\')
...
# for barId in dataArr[3].keys():
    ...
    # offset = 0.0-bar_width*(totalBarNum/2.0)-gap_width*((totalBarNum-1.0)/2)+(barCnt+0.5)*bar_width+barCnt*gap_width
    offset = 0.0-bar_width*(totalBarNum/3.0)-gap_width*((totalBarNum-1.0)/3)+(barCnt+0.5)*bar_width+barCnt*gap_width
    plt.bar(ind+offset, dataArr[2][barId], bar_width, label=barId, color='white', linewidth=2, edgecolor=colorDict[barId], hatch='///////' if barId == 'PRM-RP'  else '\\\\\\\\\\\\\\')
    barCnt += 1
...
for barId in dataArr[3].keys():
    ...
    # offset = 0.0-bar_width*(totalBarNum/2.0)-gap_width*((totalBarNum-1.0)/2)+(barCnt+0.5)*bar_width+barCnt*gap_width
    offset = 0.0-bar_width*(totalBarNum/3.0)-gap_width*((totalBarNum-1.0)/3)+(barCnt+0.5)*bar_width+barCnt*gap_width
    plt.bar(ind+offset, dataArr[3][barId], bar_width, label=barId, color='white', linewidth=2, edgecolor=colorDict[barId], hatch='///////' if barId == 'PRM-RP'  else '\\\\\\\\\\\\\\')
    barCnt += 1
...
# plt.xticks(rotation=352) #x轴标签旋转
plt.subplots_adjust(left=0.137, right=0.98, top=0.99, bottom=0.1)#图片的页边距
